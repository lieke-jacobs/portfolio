...
```{r breakout, include=FALSE}

# library(tidyverse)
library(spotifyr)
library(compmus)

tidy <- get_tidy_audio_analysis("5ZLkc5RY1NM4FtGWEd6HOE")
comp_align <- tidy |> compmus_align(beats, segments)
  ...
  mutate(
    pitches =
      map(segments,
        ...
        method = "max", norm = "aitchison"              # Change summary & norm.
      )
  ) |>
  mutate(
    timbre =
